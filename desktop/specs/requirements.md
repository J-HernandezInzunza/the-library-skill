...
| D19 | Every full-screen view roots in a global `.view` class and gets its chrome from one `<PageHeader>`, which renders **both chrome rows and the scrolling body between them**: the back row as the view's head row, the title with that page's actions as the body's first line, and the page itself in its default slot. No view draws its own back button, its own `.view__head`/`.view__body`, or its own root padding, and a back label names the **title of the page it returns to**. The catalog list is the one exception: its head is a search toolbar rather than a back row. | Five views had each written their own: two put the back button above the title and three beside it, with three different paddings, so the control visibly jumped as you navigated. Each one type-checked and looked right alone — the defect existed only *between* screens. D22 put the two rows on opposite sides of the scroll boundary, which briefly made them two components and put the duplication back: every view then repeated a nav element, a body wrapper, and a title element in the right order. One component owning all three is the only version where the order is not a thing a view can get wrong. Guarded by a source-level test for the same reason D5 and R7.6 are: nothing else can see it. |
...
| D21 | An entry's detail page states its install status in the header, using the list's own badge, and has **one** section that owns scope: the copies on this machine, each carrying its own push and remove actions. | The page asked "which copy?" three times — install radios, a push dropdown, a remove list — in three vocabularies, and never stated the fact the reader arrived with. An entry the list had just badged `installed · global` opened with a panel headed "Install", contradicted only eight sections later by "Installed copies". Attaching each action to the copy it acts on deletes the question instead of harmonising it, and gives every action the copy's real path. |
| D23 | **Installing an entry and setting it up are one page of their own**, reached from the entry page by a hand-off button. The entry page reads: name and install badge, description, source, on this machine, the hand-off, catalogs holding this name, dependencies. The guided walkthrough is started from the install page, not from the entry page. | The entry page had grown into two things: a page *about* an entry and a page for *acting on* it. The install panel and the setup readiness card sat between "what is on this machine" and "which catalogs hold this name", so reading down it crossed from facts into controls and back out into facts — which is the same complaint D21 fixed one level up, when the page opened with a panel headed "Install" and put the copies it contradicted eight sections below. Install and setup belong together because they are one job: you put a copy on the machine, then find out what it wants. Neither is what the page is *for*, which is reading an entry before acting on it (D18's argument, applied to the entry rather than the catalog). |
| D22 | The window is a frame, and **every view is that frame**: `.view` is a grid of a chrome row, one scrolling body, and an optional second chrome row, with the command bar as the app's last row under it. The document never scrolls. A view's back row and the walkthrough's composer are *rows*, not positioned elements, and no view may use `position: fixed` or `sticky` at all. | A scrolling document put the layout at the mercy of the WebView's rubber-band overscroll: a long setup transcript could be dragged past both ends, and every `position: fixed` element travelled with it — so the bar that is *defined* as the bottom of the window visibly left it, and the way out of a page was thousands of pixels above wherever the user had read to. Both `fixed` and `sticky` are approximations of "this belongs to the window": fixed rides the overscroll, and sticky pins only while its containing block is in view, so page-length chrome still moves. A row outside the scroller cannot be scrolled at all. Making the *view* the frame rather than a shell the views reach into removes every piece of plumbing between them — no injected slots, no teleports, no lifted state — and turns the invariant into something a rendered test can assert: the back row and the composer are provably not inside the scrolling body. |
...
- R4.5a Push and remove act on a named copy, never on a scope the user re-selects. Each copy row
  shows its real path, and a push names **both ends** — the copy on disk and the entry's source —

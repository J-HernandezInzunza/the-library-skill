...
| --- | --- | --- |
| ~~Source suggestion had no callable surface~~ | `_suggest_remote_for_local` existed only as a hint appended to the error that refuses a local-path source — unreachable for a GUI (never sees stderr) and backwards for the agent, which should ask before proposing an `add` rather than after being refused. | **Delivered.** `library suggest-source <path> [--json]`, backed by `_remote_suggestion`, which returns *why* there is no URL rather than a bare `None`. Exits 0 either way: no suggestion is an answer, not a failure. A directory resolves to the `SKILL.md` inside it, fixing a latent `/blob/<dir>` URL. `cookbook/add.md` now calls it instead of hand-rolling three `git` commands. 13 tests, verified by mutation. |

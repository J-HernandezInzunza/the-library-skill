...
| I deleted a skill's directory by hand and `sync` won't bring it back | By design. `sync` refreshes what is on disk; a copy you removed outside the tool is reported as *gone from disk* and left alone. `./library use <name>` puts it back, `./library uninstall <name>` drops the leftover record. The exception is a dependency: something still installed that `requires` it gets it re-fetched, and the sync report names that write and says which entry caused it. |
...
| `add` refuses a source *"inside … which is where skills install"* | The file is in `~/.claude/skills/` (or `agents/`/`commands/`), which is where installs land — the entry would overwrite its own source and leave nothing to install from. Move the content into a repository or folder you version control, add the entry pointing there, then install it so the copy under `~/.claude` comes back. `--allow-local` does not override this; it answers a different question. A *project* `.claude/skills/` path is only warned about, since it collides only when installed back into that same project. |
| `use`/`sync` fails with *"the source is the destination"* | An entry added before that refusal existed points at its own install location. Same fix: move the content somewhere you version control and `./library update <name> --set-source <new>`. The refusal is what stops the install from deleting the only copy. |
| A catalog entry shows `not_installed` although a folder of that name is in `~/.claude/skills/` | Check where it actually sits. Claude Code keeps the skills attached to your claude.ai account in `~/.claude/skills/synced/`, and parks them in `~/.claude/skills/.trash/` when that sync is off. Neither is this tool's, so neither counts as an install — and a synced skill loads as `anthropic-skills:<name>`, a different name from the catalog's. Run `./library use <name>` to install the catalog's copy alongside it. To stop the account sync entirely, set `syncClaudeAiSkills` to `false` in `~/.claude/settings.json`. |
| A skill quietly changed to another catalog's version | A pin decides what the next install fetches, so `use` or `sync` will have switched it over. `./library pin` lists every pin; `./library unpin <name>` hands the name back to registry order, and `./library use <name>` then refetches the copy that order resolves to. |
| `doctor` warns *"'\<name\>' is pinned to … which is not registered"* | The pin names a catalog that has been unregistered, or whose clone could not be read this run. The pin is inert, not broken: the name still installs, just from whatever the order resolves to instead. Re-register the catalog, or `./library unpin <name>` to drop the pin. |
| `pin` refuses with *"does not define"* | The catalog you pinned to has no entry of that name, so the pin would sit in the config doing nothing. The message lists the catalogs that do define it — pin to one of those. |
| `pin` says it did not switch the installed copy | Something about it needed a decision: local edits it would discard, a copy the tool never placed, a project install it cannot anchor to, or a dependency the newly pinned catalog cannot resolve. The pin still stands and nothing was overwritten; settle the named problem, then `./library use <name>`. |
